...
@export var horizontal_separation: int = 24
	...
	match p_what:
		NOTIFICATION_SORT_CHILDREN:		
			...
			if get_child_count() > 0:
				min_columns = int(floor(get_rect().size.x / (get_child(0).get_combined_minimum_size().x + horizontal_separation)))
...
#			Evaluate the remaining space for expanded columns/rows.
			...
			remaining_space.y -= vertical_separation * max(max_row - 1, 0)
			remaining_space.x -= horizontal_separation * max(max_col - 1, 0)
...
#			Finally, fit the nodes.
			...
			for i in range(get_child_count()):
				...
				if (col == 0):
					col_ofs = 0
					if (row > 0):
						row_ofs += (row_expand if row_expanded.has(row - 1) else row_minh[row - 1]) + vertical_separation
				...
				col_ofs += s.x + horizontal_separation
...
#	Only worry about max height, not width (since it does width automatically)
	...
	ms.y += vertical_separation * max_row
