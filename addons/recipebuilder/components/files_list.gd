...
func _ready() -> void:
	filter_edit.connect("text_changed", _on_filter_edit_text_changed)
	
	list.connect("item_clicked", _on_list_item_clicked)
	list.connect("item_activated", _on_list_item_activated)
...
func get_nice_file(file_path: String, path_bit_count: int = 1) -> String:
	var bits = file_path.replace("res://", "").replace(".tres", "").split("/")
